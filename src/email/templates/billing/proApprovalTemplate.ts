import { EmailTemplateResult, ProApprovalTemplateProps } from "../../emailTypes";
import { getEmailTemplate } from "../../emailUtils";

const PLATFORM_NAME_DEFAULT = process.env.PLATFORM_NAME || "Mitra Varta";

export function proApprovalTemplate(
  props: ProApprovalTemplateProps
): EmailTemplateResult {
  const { name, companyName, platformName = PLATFORM_NAME_DEFAULT } = props;

  const subject = "Your Pro Access Request Has Been Approved!";
  
  const text = [
    `Hello ${name || "there"},`,
    "",
    "Great news! Your Pro access request has been approved.",
    "",
  ...
  const htmlContent = `
    <h1 style="color: #10b981; margin: 0 0 20px 0; font-size: 28px;">Your Pro Access Request Has Been Approved!</h1>
    <p style="color: #334155; font-size: 16px; line-height: 1.6; margin: 0 0 16px 0;">Hello ${name || "there"},</p>
    <p style="color: #334155; font-size: 16px; line-height: 1.6; margin: 0 0 16px 0;"><strong>Great news! Your Pro access request has been approved.</strong></p>
